...
export class GameManager {
  ...
  processAction(action: PlayerAction): void {
    ...
    switch (action.type) {
      ...
      case 'COMBINED_ATTACK': {
        ...
        const { attackerIds, targetId: combinedTargetId } = action;

        if (attackerIds.length === 0) {
          this.log('[ERROR] Combined attack requires at least one attacker', 'ERROR');
          return;
        }
        ...
        for (const id of attackerIds) {
          const { card } = ZoneManager.findCard(state, id);
          if (!card || card.zone !== 'FIELD' || card.controller !== state.activePlayer) {
            this.log(`[ERROR] Invalid attacker: ${id}`, 'ERROR');
            return;
          }
          if (card.attackedThisTurn || card.keywords.has('CANNOT_ATTACK')) {
            this.log(`[ERROR] ${card.def.displayName} cannot attack this turn`, 'ERROR');
            return;
          }
          if (card.summonedThisTurn && !card.keywords.has('RUSH')) {
            this.log(`[ERROR] ${card.def.displayName} has summoning sickness`, 'ERROR');
            return;
          }
          // Combined attack: all attackers must be Vassals
          if (attackerIds.length > 1 && card.def.type !== 'VASSAL') {
            this.log('[ERROR] Combined attack only allows Vassals', 'ERROR');
            return;
          }
          attackers.push(card);
        }

        // Validate target is a Colossus on opponent's field
        const opponent = ZoneManager.getOpponent(state.activePlayer);
        const { card: target } = ZoneManager.findCard(state, combinedTargetId);
        if (!target || target.zone !== 'FIELD' || target.controller !== opponent) {
          this.log('[ERROR] Invalid combined attack target', 'ERROR');
          return;
        }
        if (target.def.type !== 'COLOSSUS') {
          this.log('[ERROR] Combined attack can only target a Colossus', 'ERROR');
          return;
        }

        const combinedEntry: StackEntry = {
          id: generateStackId(),
          type: 'DECLARE_COMBINED_ATTACK',
          sourceCard: attackerIds[0],
          controller: state.activePlayer,
          payload: { attackerIds, targetId: combinedTargetId },
          negated: false,
        };
        state.stack.push(combinedEntry);

        const totalAtk = attackers.reduce((sum, a) => sum + a.atk, 0);
        this.log(
          `${state.activePlayer} declares combined attack: [${attackers.map(a => a.def.displayName).join(', ')}] (total ATK: ${totalAtk}) → ${target.def.displayName}`,
          'ACTION'
        );
        this.emit({ type: 'COMBINED_ATTACK_DECLARED', attackerIds, targetId: combinedTargetId });
        this.openResponseWindow(combinedEntry);

        // Queue ON_ATTACK_DECLARED for each attacker
        const combinedTriggers: TriggerEntry[] = attackerIds.map(id => ({
          event: 'ON_ATTACK_DECLARED' as const,
          sourceCard: id,
          affectedCard: combinedTargetId,
          data: { attackerId: id, targetId: combinedTargetId, combined: true },
          controller: state.activePlayer,
        }));
        this.queueTriggers(combinedTriggers);
        break;
      }
  ...
  private resolveStack(): void {
    ...
    switch (entry.type) {
      ...
      case 'DECLARE_ATTACK': {
        ...
        this.resolveSingleCombat(attackerId, targetId);
        break;
      }

      case 'DECLARE_COMBINED_ATTACK': {
        const { attackerIds, targetId } = entry.payload as { attackerIds: string[]; targetId: string };
        this.resolveCombinedAssault(attackerIds, targetId);
        break;
      }
  ...
  // ---- Single Combat Resolution (1v1) -----------------------
  // Covers: Vassal vs Vassal, Colossus vs Colossus, any single attacker vs DIRECT
  // Rules:
  //   - Attacker ATK vs Defender CORE (OFFENSE) or SHIELD (GUARD)
  //   - No counter-attack: loser is simply destroyed, no retaliation damage
  //   - Tie (ATK = stat) → both destroyed
  //   - GUARD: defender can be destroyed if ATK >= SHIELD, but never retaliates
  private resolveSingleCombat(attackerId: string, targetId: string): void {
    const state = this.state;
    const { card: attacker } = ZoneManager.findCard(state, attackerId);
    if (!attacker) return;
    ...
    if (targetId === 'DIRECT') {
      ...
      this.log(
        `DIRECT ATTACK: ${attacker.def.displayName} deals ${damage} to ${opponent} (life: ${state.players[opponent].life})`,
        'ACTION'
      );
    ...
    // GUARD position uses SHIELD, OFFENSE uses CORE
    const defStat = defender.position === 'GUARD' ? defender.shield : defender.core;

    this.log(
      `COMBAT: ${attacker.def.displayName} (ATK ${atkA}) vs ${defender.def.displayName} (${defender.position === 'GUARD' ? 'SHIELD' : 'CORE'} ${defStat})`,
      'ACTION'
    );
    ...
    if (atkA > defStat) {
      ...
      this.log(`${defender.def.displayName} destroyed!`, 'EFFECT');
    } else if (atkA < defStat) {
      // Defender wins — attacker destroyed
      ZoneManager.moveCard(state, attackerId, 'ABYSS');
      this.log(`${attacker.def.displayName} destroyed!`, 'EFFECT');
    } else {
      // Tie — both destroyed
      ZoneManager.moveCard(state, attackerId, 'ABYSS');
      ZoneManager.moveCard(state, targetId, 'ABYSS');
  ...
  // ---- Combined Assault Resolution (Vassals vs Colossus) -----
  // Rules:
  //   - Vassals attack sequentially in the declared order
  //   - Each Vassal reduces the Colossus's residual CORE (or SHIELD in GUARD)
  //   - Counter-attack: after each hit, Vassal ATK vs residual stat AT THAT MOMENT
  //       - If Vassal ATK < residual → Vassal destroyed (counter-attack)
  //       - If Vassal ATK >= residual → Vassal survives, Colossus destroyed → stop
  //   - GUARD: Colossus uses SHIELD, never counter-attacks (Vassals always survive if not finisher)
  //   - Tie (last ATK = residual exactly) → all Vassals + Colossus destroyed
  private resolveCombinedAssault(attackerIds: string[], targetId: string): void {
    const state = this.state;
    const { card: colossus } = ZoneManager.findCard(state, targetId);
    if (!colossus || colossus.zone !== 'FIELD') return;

    const inGuard = colossus.position === 'GUARD';
    // Residual starts at current modified stat (respects any active modifiers)
    let residual = inGuard ? colossus.shield : colossus.core;
    const originalStat = residual;

    const totalAtk = attackerIds.reduce((sum, id) => {
      const { card } = ZoneManager.findCard(state, id);
      return sum + (card?.atk ?? 0);
    }, 0);

    this.log(
      `COMBINED ASSAULT: [${attackerIds.length} Vassals, total ATK ${totalAtk}] vs ${colossus.def.displayName} (${inGuard ? 'SHIELD' : 'CORE'} ${residual})`,
      'ACTION'
    );

    const survivors: string[] = [];
    let colossusDestroyed = false;

    for (const attackerId of attackerIds) {
      if (state.step === 'GAME_OVER') break;

      const { card: vassal } = ZoneManager.findCard(state, attackerId);
      if (!vassal || vassal.zone !== 'FIELD') continue;

      vassal.attackedThisTurn = true;
      const atkV = vassal.atk;

      this.log(
        `  ${vassal.def.displayName} (ATK ${atkV}) strikes — ${colossus.def.displayName} residual ${inGuard ? 'SHIELD' : 'CORE'}: ${residual}`,
        'ACTION'
      );

      if (atkV > residual) {
        // Vassal's ATK exceeds residual — Colossus falls, Vassal survives
        residual = 0;
        colossusDestroyed = true;
        survivors.push(attackerId);
        this.log(`  ${vassal.def.displayName} lands the killing blow! ${colossus.def.displayName} destroyed.`, 'EFFECT');
        break;
      } else if (atkV === residual) {
        // Exact match — mutual destruction: this Vassal and Colossus both die
        residual = 0;
        colossusDestroyed = true;
        // Vassal does NOT go into survivors (tie = both die)
        ZoneManager.moveCard(state, attackerId, 'ABYSS');
        this.log(`  ${vassal.def.displayName} ties with ${colossus.def.displayName} — both destroyed!`, 'EFFECT');
        break;
      } else {
        // atkV < residual — Vassal contributes damage but falls to counter-attack
        residual -= atkV;
        if (inGuard) {
          // GUARD: no counter-attack, Vassal survives
          survivors.push(attackerId);
          this.log(`  ${vassal.def.displayName} chips ${atkV} SHIELD. Residual SHIELD: ${residual}. (GUARD — no counter)`, 'EFFECT');
        } else {
          // OFFENSE: counter-attack — Vassal destroyed
          ZoneManager.moveCard(state, attackerId, 'ABYSS');
          this.log(`  ${vassal.def.displayName} chips ${atkV} CORE. Residual CORE: ${residual}. Counter-attack — ${vassal.def.displayName} destroyed!`, 'EFFECT');
        }
      }
    }

    // Resolve Colossus destruction
    if (colossusDestroyed) {
      ZoneManager.moveCard(state, targetId, 'ABYSS');
      this.log(`${colossus.def.displayName} destroyed by combined assault!`, 'EFFECT');
    } else {
      // Colossus survived — log residual stat (resets to full at end of turn)
      this.log(
        `${colossus.def.displayName} survives! Residual ${inGuard ? 'SHIELD' : 'CORE'}: ${residual} / ${originalStat} (resets end of turn)`,
        'EFFECT'
      );
    }

    this.emit({
      type: 'COMBINED_COMBAT_RESOLVED',
      attackerIds,
      targetId,
      survivors,
    });
